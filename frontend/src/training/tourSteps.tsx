...
const orderDetailSteps: TourStep[] = [
    ...
    // V18 + V17: новые фичи
    {
        ...
        content: (
            <div>
                По доставке создаются <Strong>две позиции</Strong>: «Забор» и «Отвоз». В каждой
                по умолчанию платная доставка — её можно одним кликом{' '}
                <Strong>заменить на «Самовывоз»</Strong> (кнопка «↔ Самовывоз» рядом с услугой).
            </div>
        ),
        data: { route: '/orders/3' },
    },
    {
        ...
        content: (
            <div>
                <Strong>«⚠ Проблема»</Strong> — если что-то пошло не так (потеря, спор с клиентом),
                оператор поднимает флаг с описанием. Админы сразу получают уведомление-колокольчик.
                Когда вопрос решён — кнопка «✓ Проблема решена» снимает флаг.
            </div>
        ),
        data: { route: '/orders/3' },
    },
    {
        ...
        content: (
            <div>
                <Strong>Номер квартиры</Strong> — отдельное поле возле адреса (#7). В адресе не нужен —
                иначе мешает геокодированию. Водитель видит сразу: «дом 8 к 1, кв. 25».
                При создании заказа квартира подтягивается из карточки клиента.
            </div>
        ),
        data: { route: '/orders/3' },
    },
    {
        ...
        content: (
            <div>
                <Strong>Слоты времени</Strong> подгружаются из справочника по дню недели даты
                (по умолчанию: будни 17:00-20:30, сб 10:00-20:30, вс — нет доставки).
                После выбора широкого слота можно уточнить <Strong>1-часовой подынтервал</Strong>.
            </div>
        ),
        data: { route: '/orders/3' },
    },
...
const analyticsSteps: TourStep[] = [
    ...
    },
    {
        ...
        content: (
            <div>
                В шапке вверху — <Strong>переключатель периода</Strong>: «Всё время» / «Текущий
                месяц» / 7-30-90 дней / ручной диапазон c/по. Все графики и таблицы перестраиваются.
                Клик по столбцу месяца в графике выручки — переход в заказы за тот месяц.
            </div>
        ),
        data: { route: '/analytics' },
    },
